...
int main(int argc, char *argv[]) {
  ...
  if (argc > 1) {
    // atoi()函数将数字格式的字符串转换为整数类型，需要引用头文件<stdlib.h>
    port = atoi(argv[1]);
  }
  ...
  while (true) {
    ...
    TcpRecvSendThread *cur_rs_thread = new TcpRecvSendThread(
        client); // 只有当前线程是new出来的，才能在类中使用delete this;释放空间
    bool reply = false;
    while (true) {
      cout << "Please select whether you need to manually reply to the "
              "information(y/n):";
      string reply_choose;
      cin >> reply_choose;
      if (reply_choose == "y") {
    ...
    thread cur_thread(&TcpRecvSendThread::recv_send, std::ref(cur_rs_thread),
                      reply);
    cur_thread.detach();
  }
  server.close_socket();
