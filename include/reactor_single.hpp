...
/* --------------- Declaration --------------- */
class ReactorSingle : public Reactor {
public:
    explicit ReactorSingle(const uint32_t& epoll_size);

    template <typename F, typename... Args>
    void event_loop(const int& events_num, const int& epoll_timeout,
                    BasicTcp& server,
                    const std::string& event_mode, F &&call_bcak, Args &&...args);
};
/* --------------- Declaration --------------- */


/* --------------- Definition --------------- */
inline ReactorSingle::ReactorSingle(const uint32_t& epoll_size) : Reactor(epoll_size) {}

template <typename F, typename... Args>
void ReactorSingle::event_loop(const int& events_num, const int& epoll_timeout, 
                               BasicTcp& server,
                               const std::string& event_mode, F &&call_bcak, Args &&...args) {
    ...
    while (true) {
        ...
        for (int i = 0; i < count; i++) {
            if (events_out[i].data.fd == server.sock) {
                ...
                call_bcak(events_out[i].data.fd);
            }
        }
    }
}
